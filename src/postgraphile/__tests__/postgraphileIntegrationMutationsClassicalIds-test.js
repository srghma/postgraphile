...
const mutationsDir = resolvePath(__dirname, 'fixtures/mutations-classical');
...
beforeAll(() => {
  ...
  const gqlSchemaPromise = withPgClient(async pgClient => {
    return await createPostGraphileSchema(pgClient, ['a', 'b', 'c'], { classicIds: true });
  })();
...
for (let i = 0; i < mutationFileNames.length; i++) {
  test(mutationFileNames[i], async () => {
    const result = await mutationResults[i]
    console.log(require('util').inspect(result.errors, {showHidden: false, depth: null})
